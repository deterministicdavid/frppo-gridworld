...
# z_{n+1} = z_n + h f (z_n)
def policy_mirror_stepping(mdp: GridworldMDP, tau, h: float = 1.0, grad_time_T: float = 10, annealing=False, tau_min:float = 0.0):
    num_states = mdp.num_states
    num_actions = mdp.num_actions
    tau_init = tau
...
if __name__ == "__main__":
    ...
    mdp.heatmap_plot_3V(V1=optimal_value_pia, 
                    V2=optimal_value_soft_pol_iter,
                    V3=V_mirror,
                    title1='Value fn from PIA',
                    title2='Value fn from soft PIA',
